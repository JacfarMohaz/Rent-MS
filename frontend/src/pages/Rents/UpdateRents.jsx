...
function UpdateRents() {
    ...
    const handleReadSingleRent = () => {
        axios.get(`http://localhost:5000/getsingle/rent/${params.id}`).then((res) => {
            setSn(res.data[0])
        }).catch((error) => console.log(error))
    }

    useEffect(() => {
        handleReadSingleRent()
    },[])
    ...
    return <div>
        ...
        <form className="max-w-[50em] bg-blue-500 rounded-lg p-10 mx-10 my-8">
            ...
            <button className="px-6 py-2 text-xl text-white bg-cyan-400 rounded-lg">Save Data</button>
        </form>
